...
- **static waveform** → PNG file or SVG string, with axes and time labels;
  each pixel column shows the min/max amplitude of the samples it covers, so
  peaks stay visible (a line through every n-th sample would hide them)
...
- **live visualization**: records audio (always mono; stereo is averaged)
  from an input device and displays the real-time waveform together with a
  custom transformation (lowpass filter, spectrum, signal power, ...) in a
  GUI window ([egui](https://crates.io/crates/egui)); cross-platform
